...
static char banner[] = KERN_INFO "S3C ADC driver for MINI6410, (c) 2010 FriendlyARM\n";
...
static int s3c_adc_remove(struct platform_device *pdev)
{
	...
	if (adc_clock) {
		clk_disable_unprepare(adc_clock);
		clk_put(adc_clock);
		adc_clock = NULL;
	}
...
static const struct platform_device_id s3c_adc_driver_ids[] = {
	{
		.name           = "s3c24xx-adc",
		.driver_data    = ADC_TYPE_1,
	}, {
		.name           = "s3c64xx-adc",
		.driver_data    = ADC_TYPE_2,
	}	
};
