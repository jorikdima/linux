...
static inline void stop_timer_for_1wire(void)
{
	...
	cstat &= ~S3C_TINT_CSTAT_T3INT;
	cstat |= S3C_TINT_CSTAT_T3INTEN;
	writel(cstat, S3C64XX_TINT_CSTAT);
...
{          
	...
	set_bit(fls(S3C_TINT_CSTAT_T3INT), S3C64XX_TINT_CSTAT);	
...
static struct irqaction timer_for_1wire_irq = {
	...
	.flags   = IRQF_TIMER | IRQF_IRQPOLL | IRQF_SHARED,
...
static int init_timer_for_1wire(void)
{
	...
	ret = request_irq(IRQ_TIMER3_VIC, timer_for_1wire_interrupt, IRQF_TIMER | IRQF_IRQPOLL | IRQF_SHARED, \
	      timer_for_1wire_irq.name, &timer_for_1wire_irq);
	if (ret < 0) {
		DPRINTK("ERROR to setup interrupt %d\n", ret);
		return ret;
	}
...
static void start_one_wire_session(unsigned char req)
{
	...
	cstat = __raw_readl(S3C64XX_TINT_CSTAT);
	cstat |= S3C_TINT_CSTAT_T3INTEN | S3C_TINT_CSTAT_T3INT;
	writel(cstat, S3C64XX_TINT_CSTAT);
...
static ssize_t read_proc(struct file *filp, char __user *buffer, size_t count, loff_t *ppos)
{
  	int len;
	char str[40];
	
	len = sprintf(str, "%u %u %u %u %04X %08X\n", lcd_type, firmware_ver, total_received, total_error, last_req, last_res);
	
	return simple_read_from_buffer(buffer, count, ppos, str, len);
}
...
static int __init dev_init(void)
{
	...
	if (ret == 0) {		
		...
		one_wire_timer.expires = jiffies;
		one_wire_timer_proc(0);
				
		proc_create("driver/one-wire-info", 0, NULL, &proc_fops);
	}
