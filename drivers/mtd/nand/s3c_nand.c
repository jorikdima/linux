...
#if defined(CONFIG_MACH_MINI6410) && defined(CONFIG_MTD_NAND_S3C_HWECC)
...
{	
	struct s3c2410_platform_nand *plat = dev_get_platdata(&pdev->dev);
	...
	for (i = 0; i < sets->nr_chips; i++) {
		...
		pr_info("DKDEBUG  cmdfunc 2 %lu\n", (unsigned long)nand->cmdfunc);
		...
		pr_info("DKDEBUG %lu\n", clk_get_rate(s3c_nand.clk));
		
		pr_info("DKDEBUG  cmdfunc 1 %lu\n", (unsigned long)nand->cmdfunc);
...
static int s3c_nand_suspend(struct platform_device *dev, pm_message_t pm)
{
	...
	clk_disable_unprepare(s3c_nand.clk);
	return 0;
}
