...
#define DRIVER_DESC "Samsung S3C USB High-speed/OtG device"
...
static int s3c_hsotg_probe(struct platform_device *pdev)
{
	...
	if (!hsotg->plat->phy_init) {
		dev_err(dev, "no phy init pointers initialized in platform_data\n");
		ret = -EINVAL;
		goto err_regs;	  
	}
	
	hsotg->plat->phy_init(&pdev->dev);
